...
def get_next_batch(starts, ends, bsz, seqlen, tokens, data_sharding, buf_u16, transfer_to_device=True, return_buf16=True):
...
def main():
    ...
    train_dl = make_grain_shard_loader(train_files, prefetch=256, num_threads=32, prefetch_buffer_size=256)
        ...
        try:
            ...
            while not shard_processed_fully:
                try:
                    ...
                    stacked_batch = jax.device_put(jnp.asarray(grad_accum_batch, dtype=jnp.int32), data_accum_sharding)
                    ...
                    print("Scoring model performance on vdalidation data...")
                    val_loss = 0.0
                    val_steps_count = 0
                    for shard in val_dl:
                        tokens = shard["tokens"]
                        ...
                        try:
                            ...
                            starts, ends = bf.next_batch(bsz, seqlen)
                            x, y = get_next_batch(starts, ends, bsz, seqlen, tokens, data_sharding, buf_u16=None, return_buf16=False)
